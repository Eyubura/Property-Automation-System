...
public class GetDashboardStatisticsHandler : IRequestHandler<GetDashboardStatisticsQuery, Result<DashboardDto>>
{
    ...
    public async Task<Result<DashboardDto>> Handle(GetDashboardStatisticsQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // EF Core DbContext is not thread-safe, so these queries must run sequentially.
            await GetSummaryStatistics(dashboard, cancellationToken);
            await GetRequisitionStatistics(dashboard, cancellationToken);
            await GetReceivingStatistics(dashboard, cancellationToken);
            await GetStockStatistics(dashboard, cancellationToken);
            await GetPropertyStatistics(dashboard, cancellationToken);
            await GetRecentActivities(dashboard, cancellationToken);
            await GetLowStockAlerts(dashboard, cancellationToken);
            await GetPendingTasks(dashboard, cancellationToken);
            await GetChartData(dashboard, cancellationToken);
    ...
    private async Task GetLowStockAlerts(DashboardDto dashboard, CancellationToken cancellationToken)
    {
        ...
        dashboard.LowStockAlerts = lowStockItems.Select(i => new LowStockAlertDto
        {
            ...
            Location = i.ShelfLocation != null
                ? $"{i.ShelfLocation.Aisle}-{i.ShelfLocation.Rack}-{i.ShelfLocation.ShelfNumber}"
                : "Unknown",
